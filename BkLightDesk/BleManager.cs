...
public class BleManager
{
    ...
    public async Task<string> CheckBluetoothAvailability()
    {
        try
        {
            ...
            {
                return "Bluetooth spento! Attivalo nelle impostazioni.";
            }
            return "OK";
        }
        catch (Exception ex)
        {
            return $"Errore controllo BT: {ex.Message}";
        }
    }

    // --- FIX: METODO MANCANTE AGGIUNTO QUI ---
    ...
    public void StartScanning()
    {
        ...
        {
            Log($"IMPOSSIBILE AVVIARE SCANSIONE: {ex.Message}");
            Log("Controlla che il Bluetooth sia acceso.");
        }
    }
    ...
    private async Task ConnectAndSetup(ulong address)
    {
        try
        {
            ...
            if (_device == null) { Log("Impossibile connettersi al dispositivo."); return; }

            var services = await _device.GetGattServicesAsync();
            if (services.Status != GattCommunicationStatus.Success) { Log($"Errore servizi: {services.Status}"); return; }
    ...
    public async Task SendPngAsync(byte[] pngBytes, bool useTurbo)
    {
        ...
        try
        {
            if (!await WriteAndWait(_writeChar, HANDSHAKE_FIRST, ACK_STAGE_ONE, true)) { Log("Handshake fallito."); return; }
            await Task.Delay(30); 
